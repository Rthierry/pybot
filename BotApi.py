...
import threading
import requests
from pybot.Colors import Bcolors
from pybot.SystemUtils import SystemUtils
...
class BotApi(object):
    ...
    bot_token = '244202552:AAF3Q_Ue4R6gI3g1wb_1AvBft9sVwVtio08'
    ...
    def send_message(self, command, information):
        ...
        help_message = "Vous avez besoin d'aide /help"
        error_message = "Mauvaise commande !"

        if command == BotApi.load_command:
            http_post = requests.post(BotApi.full_url + self.send_message_method, data={'chat_id': 317612641, 'text': str(information)})
            ...
            http_post = requests.post(BotApi.full_url + self.send_message_method, data={'chat_id': 317612641, 'text': help_message})
            print Bcolors.OKGREEN + "Response from Web-Server: \n" + Bcolors.ENDC + str((http_post.status_code, http_post.reason))
            ...
            http_post = requests.post(BotApi.full_url + self.send_message_method, data={'chat_id': 317612641, 'text': str(error_message)})
            print Bcolors.OKGREEN + "Response from Web-Server: \n" + Bcolors.ENDC + str((http_post.status_code, http_post.reason)) + "\n"
